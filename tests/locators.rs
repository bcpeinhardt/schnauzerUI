...
async fn locate_by_text() {
    run_script_against("locate \"Text\"", "<h1>Text</h1>").await;
}

#[tokio::test]
#[serial]
async fn locate_by_partial_text() {
    run_script_against("locate \"Te\"", "<h1>Text</h1>").await;
}
...
async fn locate_by_id() {
    run_script_against("locate \"test-id\"", "<h1 id=\"test-id\">Text</h1>").await;
}

#[tokio::test]
#[serial]
async fn locate_by_name() {
    run_script_against("locate \"test-name\"", "<h1 name=\"test-name\">Text</h1>").await;
}
...
async fn locate_by_tag_name() {
    run_script_against("locate \"h1\"", "<h1>Text</h1>").await;
}
